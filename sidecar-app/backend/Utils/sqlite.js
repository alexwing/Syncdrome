...
const getBookmarksFromDb = async (sqlitePath, volume) => {
...
const upsertBookmark = async (sqlitePath, bookmark, callback) => {
  const db = await connectToDb(sqlitePath);
...
const deleteBookmarkFromDb = async (sqlitePath, id, callback) => {
  const db = await connectToDb(sqlitePath);
  db.serialize(() => {
